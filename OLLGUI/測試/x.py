...
# ---------- 主結構 ----------
工具列框 = tk.Frame(主視窗, bg=主題樣式["light"]["工具列色"], height=30)
...
def 切換主題():
    global 目前主題
    目前主題 = "dark" if 目前主題 == "light" else "light"
    主色 = 主題樣式[目前主題]

    主視窗.configure(bg=主色["背景色"])
    工具列框.configure(bg=主色["工具列色"])
    style.configure("TNotebook", background=主色["分頁色"])
    style.configure("TNotebook.Tab", foreground=主色["文字色"])

    for 名稱, 元件 in 分頁清單.items():
        元件["frame"].configure(bg=主色["背景色"])
        for widget in 元件["frame"].winfo_children():
            if isinstance(widget, (tk.Label, tk.Entry)):
                widget.configure(bg=主色["背景色"], fg=主色["文字色"])
            elif isinstance(widget, tk.Button):
                widget.configure(bg="#cccccc", fg="black")

def 新增分頁(內容=""):
    global 分頁編號

    新頁面 = tk.Frame(分頁本, bg=主題樣式[目前主題]["背景色"])

    標籤 = tk.Label(新頁面, text="輸入內容：", font=("微軟正黑體", 12), bg=主題樣式[目前主題]["背景色"], fg=主題樣式[目前主題]["文字色"])
    標籤.pack(pady=10)

    輸入框 = tk.Entry(新頁面, width=40, font=("微軟正黑體", 12), bg=主題樣式[目前主題]["背景色"], fg=主題樣式[目前主題]["文字色"])
    輸入框.insert(0, 內容)
    輸入框.pack(pady=5)

    顯示區 = tk.Label(新頁面, text="", font=("微軟正黑體", 12), bg=主題樣式[目前主題]["背景色"], fg="blue")
    顯示區.pack(pady=10)
