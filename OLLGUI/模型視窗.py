...
def 模型設定視窗口():
    ...
    print(當前ollama開關)  # 印出當前ollama開關的值
    print(當前模型設定)  # 印出當前模型設定的值
...
###
    ...
    def 顯示已選擇的模型(event): # 當選擇模型時更新標籤的文字
        ...
        當前模型設定["查尋ollama模型結果"]=查尋模型結果選單.get() # 更新當前模型設定的值
    當前可以選的ollama模型=
    查尋模型結果選單=ttk.Combobox(查尋模型結果選單格式,state="readonly",values=當前可以選的ollama模型,font=("Arial", 10)) # 創建下拉選單
