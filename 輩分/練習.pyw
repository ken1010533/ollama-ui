...
# 创建窗口
root = tk.Tk()                             # 创建主窗口
選定模型 = tk.StringVar(value="無")        # 定義選定模型變數
root.title(f"功能丰富的 GUI 应用 - 正在使用模型: {選定模型.get()}")     # 設定視窗標題
    ...
    try:
        ...
        def 使用此模型():
            ...
            if 選定模型 not in installed_models:
                ...
                def 下載模型():
                    try:
                        subprocess.run(["ollama", "pull", 選定模型], check=True)
                        messagebox.showinfo("成功", f"模型 {選定模型} 下載完成")
                        subprocess.Popen(f"ollama run {選定模型}", shell=True)
                ...
                try:
                    ...
                    def 更新標題():
                        root.title(f"功能丰富的 GUI 应用 - 正在使用模型: {選定模型.get()}")
                        root.after(100, 更新標題)  # 每 1 毫秒更新一次標題
                except Exception as e:
...
# 按鈕
模型設定_btn = tk.Button(frame3, text="模型設定", command=模型設定)
